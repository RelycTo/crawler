...
namespace Crawler.Application.Services.Processors
{
    public abstract class BaseLinkProcessor<T> : ILinkProcessor<T> where T : ILinkParser
    {
        ...
        public async Task<IEnumerable<CrawlItemDto>> CrawlAsync(Uri uri, Uri baseUri, int taskCount, IReadOnlyCollection<string> excludedMediaTypes, ProcessStep step, CancellationToken token = default)
        {
            ...
            for (var i = 0; i < taskCount; i++)
            {
                ...
                tasks.Add(Task.Run(async () =>
                {
                    ...
                    {
                        if (!LinkRestorer.IsLinkAcceptable(currentLink, baseUri) ||
                            ProcessedLinks.ContainsKey(currentLink.AbsoluteUri))
                        {
                            continue;
                        }

                        await ProcessLinkAsync(GetResourceType(step), currentLink, excludedMediaTypes, token);
                    }
                }, token));
            }
